...
export default function App() {
  const [activeTab, setActiveTab] = useState('database'); // dashboard | database | assessment
  ...
  // Sort topics by frequency (like LeetCode typically does)
  ...
  return (
    <div className="container" style={{ maxWidth: '1300px', margin: '0 auto', padding: '2rem' }}>
      ...
      <header style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginBottom: '3rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', background: 'rgba(255,255,255,0.05)', padding: '0.5rem', borderRadius: '12px' }}>
            {['dashboard', 'database', 'assessment'].map(tab => (
              <button 
                key={tab}
                ...
                style={{
                  ...
                  border: 'none', padding: '0.6rem 1.2rem', borderRadius: '8px', cursor: 'pointer',
                  fontWeight: '500', textTransform: 'capitalize', transition: 'all 0.2s'
                }}
              >
                {tab}
              </button>
      ...
      {assessmentActive && activeTab === 'assessment' ? (
        <div className="glass" style={{ padding: '3rem', borderRadius: '24px', textAlign: 'center', minHeight: '600px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem', borderBottom: '1px solid var(--border)', paddingBottom: '2rem' }}>
            ...
            <button onClick={endAssessment} style={{ background: 'transparent', border: '1px solid var(--danger)', color: 'var(--danger)', padding: '0.8rem 1.5rem', borderRadius: '8px', cursor: 'pointer' }}>
              End Test
            </button>
          </div>
              ...
              <div key={q.id} style={{ background: 'rgba(0,0,0,0.4)', padding: '2rem', borderRadius: '16px', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>
                    <a href={`https://leetcode.com/problems/${q.title.toLowerCase().replace(/ /g, '-')}`} target="_blank" rel="noreferrer" style={{ color: 'white', textDecoration: 'none' }}>
                      {q.id}. {q.title}
                    </a>
                  </h3>
      ...
      <>
        ...
        {activeTab === 'dashboard' && (
          <div>
            <section className="glass" style={{ padding: '2rem', borderRadius: '16px', marginBottom: '2rem', display: 'flex', gap: '2rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: '300px' }}>
                <h2 style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Activity size={24} color="var(--accent-primary)" /> Sync Account
                </h2>
                <form onSubmit={fetchLeetCodeStats} style={{ display: 'flex', gap: '1rem' }}>
                  ...
                  />
                  <button type="submit" disabled={loading} style={{ background: 'var(--accent-primary)', border: 'none', color: 'white', padding: '0.8rem 1.5rem', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
                    {loading ? 'Fetching...' : 'Sync LeetCode'}
                  </button>
                </form>
              </div>
            ...
            <h2 style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Award size={24} color="var(--accent-secondary)"/> Topic Progress Tracker</h2>
        ...
        {activeTab === 'database' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            {/* Leetcode Style Topic Pill Cloud & Filters */}
            <div className="glass" style={{ padding: '1.5rem', borderRadius: '16px' }}>
               <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                 ...
                 <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, minWidth: '250px' }}>
                    ...
                    <input 
                      type="text" 
                      placeholder="Search question name or LeetCode number..." 
                      value={searchQuery}
                 ...
                 <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                   ...
                   <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                     <select 
                        ...
                        style={{ padding: '0.6rem', borderRadius: '8px', background: 'rgba(0,0,0,0.5)', color: 'white', border: '1px solid var(--border)', outline: 'none', fontSize: '0.9rem' }}
                      >
               ...
               <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.8rem' }}>
                  <button 
                    ...
                    style={{ 
                      padding: '6px 12px', border: 'none', borderRadius: '20px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px',
                      background: selectedTopic === 'All' ? 'var(--text-primary)' : 'rgba(255,255,255,0.1)',
                      color: selectedTopic === 'All' ? 'black' : 'var(--text-primary)',
                      transition: 'all 0.2s'
                    }}
                  >
                    All Topics
                  ...
                  {sortedTopics.map(topic => {
                     const topicCount = questionsData.filter(q => q.topics.includes(topic)).length;
                     ...
                     return (
                        <button 
                          ...
                          style={{ 
                            padding: '6px 12px', border: 'none', borderRadius: '20px', cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px',
                            background: isSelected ? 'var(--text-primary)' : 'rgba(255,255,255,0.1)',
                            color: isSelected ? 'black' : 'var(--text-primary)',
                            transition: 'all 0.2s'
                          }}
                        >
                          {topic} <span style={{ opacity: isSelected ? 0.8 : 0.5 }}>{topicCount}</span>
                        </button>
                     )
                  })}
               </div>
                 ...
                 <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                   ...
                   <tbody>
                     {questionsData
                       ...
                       .map(q => (
                       <tr key={q.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', transition: 'background 0.2s', ':hover': {background: 'rgba(255,255,255,0.05)'} }}>
                           ...
                           <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                             ...
                             <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                               {q.topics.slice(0, 3).map(t => (
                                 <span key={t} style={{ fontSize: '0.7rem', color: 'var(--accent-secondary)', background: 'rgba(99, 102, 241, 0.1)', padding: '0.1rem 0.5rem', borderRadius: '4px' }}>{t}</span>
                               ))}
                             </div>
                           </div>
                         </td>
                         ...
                         <td style={{ padding: '1rem' }}>
                           <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                             {q.companies.slice(0, 3).map(c => (
                               <span key={c} style={{ fontSize: '0.7rem', background: 'rgba(255,255,255,0.05)', padding: '0.2rem 0.6rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }}>{c}</span>
                             ))}
        ...
        {activeTab === 'assessment' && !assessmentActive && (
          <div className="glass" style={{ padding: '4rem 2rem', borderRadius: '16px', maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.2rem', marginBottom: '3rem', lineHeight: '1.6' }}>
              Simulate a real online assessment. Pick a LeetCode topic, set your timer, and solve unseen questions without distractions.
            </p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '400px', margin: '0 auto' }}>
              <div style={{ textAlign: 'left' }}>
                ...
                <select 
                  ...
                  style={{ width: '100%', padding: '1rem', borderRadius: '12px', background: 'rgba(0,0,0,0.5)', color: 'white', border: '1px solid var(--accent-primary)', outline: 'none', fontSize: '1rem' }}
                >
              ...
              <div style={{ textAlign: 'left' }}>
                ...
                <select 
                  ...
                  style={{ width: '100%', padding: '1rem', borderRadius: '12px', background: 'rgba(0,0,0,0.5)', color: 'white', border: '1px solid var(--border)', outline: 'none', fontSize: '1rem' }}
                >
              ...
              <button 
                ...
                style={{ background: 'var(--accent-primary)', border: 'none', color: 'white', padding: '1.2rem', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1.1rem', marginTop: '1rem', transition: 'transform 0.2s', ':hover': {transform: 'scale(1.02)'} }}
              >
                Start Assessment
              </button>
            </div>
